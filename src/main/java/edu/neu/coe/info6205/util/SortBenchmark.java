...
//    }
    ...
    int limit = 10000;
    while (limit <= 260000) {
        System.out.println("Number of elements: " + limit);
        Helper<Integer> helper = HelperFactory.create("Heap Sort", limit, config);
        Sort<Integer> sort = new HeapSort<>(helper);
        sort.init(limit);
        Integer[] xs = helper.random(Integer.class, r -> r.nextInt(10000));
        helper.preProcess(xs);
        
        if (config.getBoolean("helper", "instrument")) {
